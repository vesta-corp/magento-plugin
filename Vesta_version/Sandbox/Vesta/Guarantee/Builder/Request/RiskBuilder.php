...
class RiskBuilder
{
    ...
    private function getShippingXml()
    {
        ...
        $InfoData .= "</Delivery></ShoppingCart><CustomMerchantData version='0.1'>
        <CrossBorderFulfillment>
        <ExchangeRate>{$this->getCurrentCurrencyRate()}</ExchangeRate>
        <ReceiveAmount>{$this->order->getPayment()->getAmountOrdered()}</ReceiveAmount>
        <ReceiveCurrency>{$this->getCurrentCurrencyCode()}</ReceiveCurrency>
        <SendAmount>{$this->order->getGrandTotal()}</SendAmount>
        <SendCurrency>{$this->order->getOrderCurrencyCode()}</SendCurrency>
    </CrossBorderFulfillment>
        </CustomMerchantData>
";
