...
class RiskDataBuilder
{
        ...
        if(empty($shipping_method)){
            ...
            /* if (preg_match('/[A-Za-z].*[0-9]|[0-9].*[A-Za-z]/', $shipping_method)) {
                ...
                $infoData = "<ShippingCarrier>{$get_shipping[0]}</ShippingCarrier>
                        ...
                        <ShippingClass>{$get_shipping[0]}</ShippingClass>";
            } else {
                $infoData = "";
            } */
            $infoData = "<ShippingCarrier>{$get_shipping[0]}</ShippingCarrier>
    ...
    public function isPdof($order = null, $payment = null)
    {
        if ($this->session->isLoggedIn()) {
            ...
            $cclast4 = $payment_info['cc_number'];
            $month = $payment_info['cc_exp_month'];
            $year = $payment_info['cc_exp_year'];
