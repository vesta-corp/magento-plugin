...
class CustomerDetailsBuilder implements BuilderInterface
{
    ...
    public function build(array $buildSubject)
    {
        ...
        return [
            self::ADDRESS_ONE => substr($billingAddress->getStreetLine1(),0,30),
            self::ADDRESS_TWO => ($billingAddress->getStreetLine2() != null) ? substr($billingAddress->getStreetLine2(), 0, 30) : '',
            self::CITY => substr($billingAddress->getCity(), 0, 30),
            self::COUNTRY_CODE => $billingAddress->getCountryId(),
            self::FIRST_NAME => substr($billingAddress->getFirstname(), 0, 20),
            self::LAST_NAME => substr($billingAddress->getLastname(), 0, 20),
            self::POSTAL_CODE => substr($billingAddress->getPostcode(), 0, 10),
            self::REGION => substr($billingAddress->getRegionCode(), 0, 30),
