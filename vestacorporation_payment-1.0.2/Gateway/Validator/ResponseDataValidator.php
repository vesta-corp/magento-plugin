...
class ResponseDataValidator extends AbstractValidator
{
    ...
    public function validate(array $validationSubject)
    {
        ...
        if ($this->isSuccessfulTransaction($response)) {
            ...
            }elseif(isset($response[self::RESPONSE_CODE]) && $response[self::RESPONSE_CODE] == "0" &&
            isset($response[self::PAYMENT_RESPONSE]) &&
            in_array($response[self::PAYMENT_RESPONSE], self::DECLINE_STATUS)){
                $this->responseCapture($response, $response[self::PAYMENT_RESPONSE]);
            } else {
    ...
    private function responseCapture($parseResult = [])
    {
        if (isset($parseResult['ResponseCode']) && isset($parseResult['PaymentStatus'])) {
            ...
            switch ($parseResult['PaymentStatus']) {
                case "1":
                    $message = "Card decline by bank";
                    break;
                case "3":
                $message = "Card decline by acquirer";
                    break;
            }
            throw new LocalizedException(
                __($message)
